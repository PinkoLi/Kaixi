...
//    UIImageView *imgView = [[UIImageView alloc] initWithFrame:CGRectMake(128, 0, 768, 1024)];
...
- (UIScrollView *)scrollViewWithPosition:(MRImgLocation)imgLocation withImgView:(UIImageView *)imgView{
    ...
    double version = [[UIDevice currentDevice].systemVersion doubleValue];
    if(version<8.0){
        scrollView.frame=CGRectMake(width * imgLocation,0,width,height);
        
    }else{
        scrollView.frame=CGRectMake(1024 * imgLocation, 0, 1024,768);
    }
...
- (void)scrollViewDidEndDecelerating:(UIScrollView *)scrollView
{
    ...
    if (self == scrollView) {
        ...
        [scrollView setContentOffset:CGPointMake(1024, 0) animated:NO];
...
//        
//       //  弹出判断
//        if (height*0.5 < offsetY || offsetY < height*1.8) {
//            [_scrCenter setZoomScale:kImgZoomScaleMin animated:NO];
//        }
//        NSLog(@"csdasdssdfsdfsdfsadf");
//    }
}
...
- (void)doubleTapClick:(UITapGestureRecognizer *)tap{
    ...
    if (_scrCenter.zoomScale > kImgZoomScaleMin) {
        ...
        NSLog(@"imgLocation:%ld",[self imgLocation]);
    }
}
@end
